...
| `missing_iv_candidate` | blocking | identification | An instrument is the only route left and the graph holds none. Produced since #450 by the one question whose honest answer is an instrument: a treatment declared to be in a reciprocal loop with its outcome (`feedback`), where the treatment is endogenous by construction, no adjustment set closes that, and the front-door escape is gone too because every mediator on an X-to-Y path sits inside the loop. It had **no producer** before that, and the reason is worth keeping: not effort, but that no question in the kernel had an instrument as its answer — the old classifier read for a *failed* IV derivation step, and the kernel writes only steps that succeeded. The three routes are the instrument, resolving the loop in time (a stronger model, and no instrument needed), and withdrawing the loop as a deliberate premise. |
...
| `transport_sources_disagree` | blocking | point_estimate | Two or more declared source domains each transport the same target effect and carry it to different numbers. Theta is declared rather than estimated, so the difference cannot be sampling noise: the distributions supplied refute at least one declared selection diagram. A falsification in the same family as `overidentification_rejected` — the kernel reports no number at all, because reporting either one chooses which diagram to believe on the caller's behalf. The only member of `MISSING_ITEM_GAPS` that offers no `alternative_paths`: every other species names something a caller could go and get, and the repair here is to withdraw or correct a declaration. |
...
| `proxy_coarsening_undeclared` | blocking | point_estimate | A proximal query posits `k` states for the unobserved confounder and a proxy column presents some other number of levels, with no `proxy_coarsening` on the query. Miao's formula (5) inverts a k×k measurement channel, so the two have to agree — and folding a finer proxy IS sound (a conditional independence survives any function of the variable it holds for, so a grouped proxy still satisfies the model-(f) criteria, and in the population every grouping whose folded channel keeps full rank identifies the same effect). What no data settles is WHICH grouping: two groupings of the same sample give two numbers. So the estimator refuses rather than guessing, and this is that refusal said as an errand — the missing thing is a declaration, and the two routes are the two branches (k is right and the proxy is finer, or the proxy is showing that k was posited too small). Once declared, the grouping enters the assumption ledger with provenance `caller_chose` and the verifier re-runs the fold from the recorded per-level counts. Distinct from `declared_type_data_mismatch`, where a column contradicts its own declaration: here the column and its declaration agree and it is the estimand's `k` they do not match. |
| `feedback_loop_reaches_the_estimand` | blocking | point_estimate | #450. A declared `feedback` loop can influence this query's treatment or outcome, and it is NOT between the two of them — so the two-equation reduction an instrument rescues (Haavelmo 1943) does not apply, that result being about a system of two equations. Its own kind rather than `unidentifiable_no_admissible_set` because it says something stronger and earlier: a cyclic SCM need not have a solution, and where it does the interventional distribution need not be unique (Bongers, Forré, Peters & Mooij 2021, Ann. Statist. 49(5):2885-2915), so the quantity a DAG would identify may not exist here to be identified. Nothing to collect, therefore: the routes are to resolve the loop in time or to withdraw it. The reach test is deliberately conservative — a loop that can influence neither end is left alone, and one that can is withdrawn even where the algebra of its particular shape would have survived. |
| `regularisation_is_moving_the_answer` | important | interpretation | #451. A continuous-proxy proximal query solves the outcome bridge `E[h(W,X)\|Z,X] = E[Y\|Z,X]`, a Fredholm integral equation of the first kind — ill-posed, so no numeric solution exists without a penalty, and the penalty is a choice nothing in the data settles (Miao, Geng & Tchetgen Tchetgen 2018 *Biometrika* 105(4) §3; sieve two-stage form after Deaner 2018 arXiv:1807.02667 and Cui, Pu, Miao, Zhang & Tchetgen Tchetgen 2024 *JASA* 119(546)). The estimator re-solves at four penalties spanning eight orders of magnitude and files this when the answer BENDS — the point differs from the least-penalised solved rung by more than one standard error — or when a lighter rung has no solution at all. A caveat and not an ask: a number WAS produced and is re-derivable from the recorded moments; what is impaired is reading it as a fact about the world rather than about λ. Distinct from `proxy_coarsening_undeclared`, the discrete regime's refusal: there nothing was computed and a declaration is missing, here everything was computed and one input of it was nobody's measurement. Spread alone is deliberately NOT the criterion — a bridge whose penalty barely moves the answer can still show a wide spread across rungs when the extreme rung is far outside the range anyone would use. |
| `treatment_bridge_leaves_its_range` | important | interpretation | #455. A proximal query that declares a TREATMENT bridge solves `E[q(Z,a,X)\|W,A=a,X] = 1/f(A=a\|W,X)` for `q` (Cui, Pu, Miao, Zhang & Tchetgen Tchetgen 2024 *JASA* 119(546), Theorem 2.2). `q` is a reciprocal probability and is therefore at least one everywhere. Themis solves for it over a sieve LINEAR in its parameters — a deliberate departure from the paper's `1 + exp{...}` working model, taken because only a linear-in-parameters `q` leaves finite sufficient statistics a verifier can re-solve it from without the data — and a linear sieve does not know that its target is bounded below. Where the declared span cannot hold a function of that shape, the fit dips below zero, and a row with a negative `q` contributes a NEGATIVE weight to the inverse-probability average. Filed when the share of such rows in either arm exceeds 0.5%. A caveat and not an ask: a number WAS produced and more rows do not repair a span, so the two routes are to widen the treatment bridge or to read the doubly robust answer, which does not divide by this bridge alone and is already on the envelope. Distinct from `regularisation_is_moving_the_answer`, the other continuous-regime caveat: there the penalty is doing the work, here the declared function class cannot reach the function.  |

| `answer_is_a_test_not_an_effect_size` | blocking | point_estimate | #457. A discrete proximal query whose channel cannot be inverted, where Miao, Geng & Tchetgen Tchetgen 2018 *Biometrika* 105(4) §4's test of the causal null `H0 : X ⊥ Y \| U` still runs. Formula (5) needs both proxies at `k` levels and needs `P(W\|Z,x)` to invert; §4 needs only that the channel STACKED across the treatment's levels have full row rank, which is strictly weaker — a per-`x` matrix can be singular while the stack is not. So three refusal species hand over rather than ending the run: `proxy_cardinality_mismatch` (only where the treatment-side proxy is too COARSE — too fine is an errand that reaches a number, and keeps its refusal), `treatment_not_binary`, and `rank_condition_violated`. The two proxies are used at opposite granularities and that asymmetry is the shape of the thing: `W` folds to `k` because γ carries one coefficient per state of the latent, while `Z`'s levels are spent as moments and are taken at whatever resolution they come in. A caveat and not an ask, for the reason `answer_is_bounds_not_point_estimate` is: an answer of a weaker shape came back, and the first thing the reader needs is not to read it as the stronger one — a p-value handed to someone who asked how much reads as a small effect. Rejecting is evidence of causation with no statement of size, sign or subgroup; failing to reject is not evidence of no effect. The tier is reconciled to `none`, since a test is neither a point nor an interval. Distinct from `unidentifiable_no_admissible_set`: the graph identifies the effect here and the SAMPLE cannot deliver it. |
...
One of those eight has no classifier:
``missing_population_distribution``, which never had a signal, declared in
``data_gap_report.GAP_KINDS_WITH_NO_PRODUCER``. ``missing_iv_candidate``
was the other until #450 and left by the door that note describes — a
question whose honest answer is an instrument, which is what a declared
reciprocal loop between treatment and outcome poses.
...
outcome fit), ``proxy_coarsening_undeclared`` (post-proximal
cardinality refusal — the one filed where the estimator produced NO
number, which is why it is blocking rather than a caveat),
``regularisation_is_moving_the_answer`` (post-proximal-bridge penalty
ladder — the other side of that pair, filed where a number DID come
out and one of its inputs was nobody's measurement) and
``treatment_bridge_leaves_its_range`` (post-proximal-bridge range
check on the treatment bridge, filed where the fitted reciprocal
propensity went negative and the weights stopped being weights).
