...
| `unattempted_layer_due_to_dispatch_conflict` | important | interpretation | Query specified multiple identification layers (e.g. mediator + target_population) but kernel dispatched only one; the other was silently skipped (mediation × transport must be sequential per Cole & Stuart 2010). |
