...
```text
14841 passed / 512 skipped, warning-clean
```
...
**代价，量过再说，也说清没换什么。** 那三个弯法原来在校验器那一层就被快速拒掉，
现在它们过得了校验、要跑完整条验证链——所以贵的不是「多问了几遍」，是**问题
第一次真的被问到了**。同一棵树、同一台机、中间不叠别的任务，
`-n 6 --dist loadgroup` 全量背靠背：**1097 s（18:17）→ 1548 s（25:48），+41%**。
**只有这个比值算数**（本机每个 python 进程只跑到约三分之一个核心，绝对墙钟
不可移植）。杠杆留在 `_PER_LEAF`，**但没有动它**：把三个降成两个是拿覆盖换墙钟，
而这个仓库的优先级是静默答错 > 跑得慢。
