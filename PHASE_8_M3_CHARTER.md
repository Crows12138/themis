...
- ~~NOTEARS~~ —— **已做（#462）**。当初推迟的理由是「连续优化的解重放不了，所以验不了」，这条理由**是错的**：目标函数与梯度只经由 Gram 矩阵 S = X'X/n 依赖数据，所以一个 d×d 矩阵就是整个问题的充分统计量，无环性残差 / 目标值 / 一阶残差全都能被一个没跑过求解器的东西重算出来。**全局最优仍然不可证**——不凸——信封如实说了，没有假装。
- DAG-GNN / RL-discovery（同一族的其余成员）——它们没有上面这条充分统计量，重放不了也重算不了，所以推迟的理由在它们身上仍然成立
