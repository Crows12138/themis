"""A generic property: every SumExpr.bind name MUST appear as a
VarRef somewhere in the bound atom's value within its body.
Otherwise the sum is degenerate — the bind binds nothing the body
reads — and a degenerate sum evaluates to one term of the sum it
was supposed to take, which looks like an answer.

The shape this catches: a `_build_q_factor` shortcut that produced
...
Applied to the Tian Lines 1-6 formulas — which the suite had long
claimed work — this helper flagged degenerate sums in BOTH the
pure-DAG chain AND the disjoint-Y-component cases. Same root cause:
...
The fix: split ``_IdState.x`` (algorithmic recursion variable,
...
def find_unbound_varrefs(formula: FormulaExpr) -> list[str]:
    ...
    The sister property to find_degenerate_sums: degenerate
...
# ---------------------------------------------------------------------------
# Self-test: the helper itself catches the buggy shape
# ---------------------------------------------------------------------------


def test_helper_catches_degenerate_sum_synthetic():
    """Hand-construct the buggy shape and confirm the helper
...
# ---------------------------------------------------------------------------
# find_unbound_varrefs self-tests
# ---------------------------------------------------------------------------
...
def test_tian_pure_chain_no_unbound_varrefs():
    """The Tian pure-chain formula must be
...
def test_tian_disjoint_y_no_unbound_varrefs():
    """The disjoint-Y formula must be fully
...
def test_backdoor_chain_rule_no_unbound_varrefs():
    """Backdoor with 2-Z chain-rule expansion must be
    fully scoped."""
...
def test_frontdoor_two_mediators_no_unbound_varrefs():
    """Front-door 2-mediator must be fully scoped."""
...
# ---------------------------------------------------------------------------
# Numerical evaluation pin: this section evaluates real Tian formulas
# against a concrete Theta to check the math is RIGHT, not just the
# structure. A degenerate sum leaves the formula looking well-formed
# and evaluating wrong;
...
def test_tian_pure_chain_evaluates_to_correct_ate():
    ...
    assert abs(actual - expected) < 1e-9, (
        ...
        f"With a degenerate sum it gives {0.8} (P(Y|X=T,M=T) — "
...
def test_tian_disjoint_y_evaluates_to_correct_ate():
    ...
    correctly into all body factors; degenerate, they would return
    the constant P(y=T|z1=T, z2=T)."""
    from themis.runtime.c_factor import identify_via_tian
    from themis.runtime.numeric_estimator import (
        ProbabilityKey, Theta, estimate_formula,
    )
    ...
    assert abs(actual - expected) < 1e-9, (
        ...
        f"{expected}. Degenerate, it returns 0.9 "
...
# necessary but not sufficient — a formula can pass the structural
# audit and still evaluate wrong. Pin them with
