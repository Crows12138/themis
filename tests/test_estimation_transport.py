...
def test_multi_z_rejects_marginal_not_summing_to_one():
    with pytest.raises(EstimatorFailure) as exc:
        estimate_transport(
            _multi_z_source(n=500, seed=0),
            treatment="x", outcome="y", adjustment=("z1", "z2"),
            target_marginal=_joint_target(0.4, 0.1, 0.2, 0.1),  # 0.8 ≠ 1
            ci_bootstrap=0,
        )
    assert exc.value.failure_type == "probabilities_do_not_sum"
...
def test_multi_z_rejects_predicate_set_mismatch():
    ...
    with pytest.raises(EstimatorFailure) as exc:
        estimate_transport(
            _multi_z_source(n=500, seed=0),
            treatment="x", outcome="y", adjustment=("z1", "z_other"),
            target_marginal=_joint_target(0.25, 0.25, 0.25, 0.25),
            ci_bootstrap=0,
        )
...
def test_rejects_malformed_target_marginal():
    ...
    assert exc.value.failure_type == "malformed_argument"
