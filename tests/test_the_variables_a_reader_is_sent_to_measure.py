...
def test_the_items_that_send_a_reader_to_measure_something():
    ...
    288 of the 373 skeletons are variable patches, which name a predicate
    ...
    assert len(skeletons) == 373, len(skeletons)
    kinds = {}
    for sk in skeletons:
        kinds[sk.get("kind")] = kinds.get(sk.get("kind"), 0) + 1
    assert kinds == {"variable_patch": 288, "probability": 85}, kinds
