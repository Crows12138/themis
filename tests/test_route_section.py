...
ROUTES = {
    ...
    blocks.TRANSPORT_IDENTIFICATION: (
...
def test_the_fixtures_cover_the_family():
    ...
    renderer says."""
    assert set(ROUTES) == set(blocks.declared_as(blocks.ROUTE))
...
def test_the_route_sits_between_the_answer_and_the_model():
    ...
    payload, _ = ROUTES[blocks.IDENTIFICATION]
    report = build_analysis_report(
        _result(**{blocks.IDENTIFICATION: payload}),
        program={"statements": []},
    )
...
def test_routes_are_stated_in_registry_order():
    ...
    ident, _ = ROUTES[blocks.IDENTIFICATION]
    missing, _ = ROUTES[blocks.MISSING_DATA_RECOVERY]
    result = _result()
    # Inserted the other way round on purpose.
    result["extensions"][blocks.MISSING_DATA_RECOVERY] = missing
    result["extensions"][blocks.IDENTIFICATION] = ident
...
def test_a_route_does_not_repeat_what_the_line_above_it_already_said():
    ...
    text = _render_route(_result(**{blocks.IDENTIFICATION: ident,
                                    blocks.IV_IDENTIFICATION: iv}))
...
def test_an_unrecognised_pattern_is_named_rather_than_dropped():
    ...
    text = _render_route(
        _result(**{blocks.IDENTIFICATION: {"pattern": "some_new_criterion"}})
    )
