...
export const ASSUMPTION_CLAIM_WORDS: Record<string, Words> = {
  ...
  confusion_matrix_from_a_validation_study_on_: {
    zh: '{suffix} 的混淆矩阵由一次验证研究数出来，计数表已声明——bootstrap 每一轮按每一列的 Dirichlet 重抽它，所以区间同时携带主样本与那次计数两份不确定性。被信的不再是「这个矩阵是对的」，而是「那份计数是对的、验证受试者的真实状态确实已知、他们和主样本适用同一条通道」；矩阵仍进入校正本身，所以它错了点估计仍然错',
    en: 'the confusion matrix on {suffix} was counted in a validation study whose tally is declared — each bootstrap round redraws it from each column\'s Dirichlet, so the interval carries that study\'s uncertainty as well as the main sample\'s. What is trusted is no longer that the matrix is right but that the tally is, that the validation subjects\' true states really were known, and that the same channel applies to them and to the main sample; the matrix still enters the correction itself, so if it is wrong the point estimate is still wrong',
  },
  ...
  confusion_matrix_known_and_fixed_on_: {
    zh: '{suffix} 的混淆矩阵已知且固定（协议规定的编码规则、厂商标称的错分率）——它进入校正本身（p_true = M⁻¹ p_obs），所以它错了错的是点估计，不只是区间宽度；区间只算主样本这一份不确定性',
    en: 'the confusion matrix on {suffix} is known and fixed (a coding rule set by protocol, an error rate quoted by the maker) — it enters the correction itself (p_true = M⁻¹ p_obs), so if it is wrong the point estimate is wrong and not only the width of the interval; the interval prices the main sample alone',
  },
...
export const BOUNDS_NOTE_WORDS: Record<string, Words> = {
  a_sharper_method_was_declined_for_scale: {
    zh: '图里有工具 {instrument}，本来能给出这一臂上的 Balke-Pearl 锐界，但在 {treatment_levels}×{outcome_levels}×{instrument_levels} 个水平下它的响应函数划分有 {treatment_levels}^{instrument_levels}·{outcome_levels}^{treatment_levels} 种类型，超过本实现能解的 {cap} 种。这里给的是不加假设的下限区间 —— 报它是因为更紧的方法按规模被放弃了，不是因为没有更紧的方法。把某个变量的水平合并粗一些，锐界就又够得着了。',
    en: 'the graph has an instrument {instrument}, so a Balke-Pearl sharp bound on this arm was available, but at {treatment_levels}×{outcome_levels}×{instrument_levels} levels its response-function partition has {treatment_levels}^{instrument_levels}·{outcome_levels}^{treatment_levels} types, past the {cap} this implementation can solve. What is reported here is the assumption-free floor — reported because the sharper method was declined on size, not because there is no sharper method. Coarsen one variable\'s levels and the sharp bound is reachable again.',
  },
  one_side_tightened: {
...
export const GAP_DESCRIBES: Record<string, Words> = {
  ...
  the_sample_is_restricted_on_a_collider: {
    zh: '样本被结构性限制为 `{collider}={value}` 的受试者（program 里有 ObservationStatement 编码了这个限制），而在 `{intervention}` 与 `{target}` 之间有一条在 `{collider}`（或它的某个祖先）相撞的路径（两条臂可经潜在/双向边，即 M-bias）。限制样本就是在做条件：用『仅 {collider}={value} 的子样本』估计 P({target} | do({intervention})) 会打开这条非因果路径，给估计引入 selection-induced bias。Hernán-Hernández-Díaz-Robins 2004 *Epidemiology* 15:615 "A Structural Approach to Selection Bias" 的结构。',
    en: 'the sample is structurally restricted to subjects with `{collider}={value}` (an ObservationStatement in the program encodes that restriction), and between `{intervention}` and `{target}` there is a path that collides at `{collider}` or at a node `{collider}` descends from (either arm may run through a latent or bidirected edge, which is M-bias). Restricting a sample is conditioning on it: estimating P({target} | do({intervention})) from the {collider}={value} subsample alone opens that non-causal path, putting selection-induced bias into the estimate. This is the structure of Hernán-Hernández-Díaz-Robins 2004 *Epidemiology* 15:615 "A Structural Approach to Selection Bias".',
  },
...
export const GAP_ROUTES: Record<string, Words> = {
  ...
  maybe_it_is_not_a_common_effect: {
    zh: '如果 `{intervention}` 与 `{target}` 之间实际并没有在 `{collider}` 相撞的路径，更新 DAG 删掉这条路径上的一条边（因果边或双向边）—— 当前结构性结论会随之改变',
    en: 'if no path between `{intervention}` and `{target}` in fact collides at `{collider}`, update the DAG and remove an edge on that path (a cause or a bidirected edge) — the structural conclusion moves with it',
  },
...
export const REFUSAL_SAYS: Record<string, Words> = {
  ...
  matrix_set_declared_two_ways: {
    zh: '沿 {axis} 变化的这组混淆矩阵里，{counted} 这些层给的是验证计数、{fixed} 这些层给的是矩阵本身。一条通道要么是数出来的、区间带着那次计数的不确定性，要么是精确给定的、区间只算主样本——两者混在一起产生的区间不是其中任何一个，而页面上是同样两个数',
    en: 'in this set of confusion matrices varying along {axis}, the levels {counted} were declared as validation tallies and the levels {fixed} as matrices. A channel is either counted, and the interval carries that counting, or exact, and the interval prices the main sample — an interval mixing the two is neither, and it is the same two numbers on the page',
  },
  matrix_wrong_shape: {
  ...
  validation_counts_unusable: {
    zh: '{what}声明成了一份验证研究的计数表，但收到的 {given} 不是一个由非负有限计数组成的矩形。区间要按每一列的 Dirichlet 重抽这个矩阵，说不出计数就说不出分布',
    en: '{what} was declared as a validation study\'s count table, and {given} is not a rectangle of non-negative finite counts. The interval redraws the matrix from each column\'s Dirichlet, and a tally it cannot read names no distribution',
  },
  validation_state_never_observed: {
    zh: '{what}的验证计数表里，第 {columns} 列（按真实状态的顺序）一个受试者都没有。那一列本该是「真实状态是它的人被记成各个状态的比例」，没有人站在那个状态上，这个比例就不是任何东西的比例',
    en: 'the validation count table for {what} has no subjects at all in column(s) {columns} (in true-state order). Each column is the proportions with which subjects at that true state were recorded, and with nobody standing there it is a proportion of nothing',
  },
}
...
export const SEAMS: Record<string, Words> = {
  ...
  discovery_asks: BETWEEN_SENTENCES,
