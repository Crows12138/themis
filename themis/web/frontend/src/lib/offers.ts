...
//
// One fact, because there is one: whether there is a model behind this
// deployment. Three surfaces need one — asking in prose, the AI-priors
// fallback, and writing a result up as a reply — and everything else on
// this page is the kernel's, which holds no model. A roster of feature
// names would be a second place to forget an entry; what the three share
// is not a name.
//
...
void fetchOffers().then(settle, () => settle({ llm: false }))
