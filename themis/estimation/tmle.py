...
def estimate_tmle_ate(
    ...
    *,
    ...
    return TMLEEstimate(
        ...
        form=form,
        identification_assumptions=_identification_specs_tmle(len(adjustment)),
        cluster=cluster,
    )
...
def _identification_specs_tmle(n_adj: int) -> tuple[dict, ...]:
    specs = [
        ...
        {"id": "positivity_overlap_of_treatment_arms",
         "claim": "重叠 / positivity：每个调整集层内处理组与对照组都有样本（TMLE 对稀薄重叠比 g-formula 稳，但仍需重叠）",
