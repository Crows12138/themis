...
Exposes Themis' seven public JSON-in/JSON-out entry points as MCP tools
(``themis_run`` / ``themis_apply_patch_and_run`` / ``themis_verify`` /
``themis_verify_data_gap_report`` / ``themis_estimate`` /
``themis_discover`` / ``themis_list_resources``) and twelve prompt /
schema files as MCP resources, so any MCP-capable LLM client (Claude
Code, Claude Desktop, etc.) can drive the kernel without copy-pasting
prompts and inputs by hand.
