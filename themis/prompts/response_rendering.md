...
| `proximal_bridge` | the SAME identification argument as `proximal_matrix` for CONTINUOUS proxies, where there is no channel matrix to invert. What is solved for instead is the outcome bridge `h`, the function satisfying `E[h(W,X)\|Z,X] = E[Y\|Z,X]`, after which the answer is `E[h(W,1)] − E[h(W,0)]`. That equation is ill-posed — it has no numeric solution without a stabilising penalty — so a penalty λ is ALWAYS in the answer, either the caller's or one the estimator scaled to the problem. Say which, say what it was, and read `penalty_ladder` in the derivation before calling the number stable: it re-solves at four penalties spanning eight orders of magnitude, and the gap `regularisation_is_moving_the_answer` fires where the answer bends further than sampling noise. The bridge is assumed to lie in the span of a declared design (`outcome_terms`), which is an assumption about function shape rather than about the graph — more data does not fix a bridge outside that span, so do not present the point as non-parametric. A design is a SUM OF TERMS and a term is a tensor product of one basis per variable it names, so read it before describing the answer: several outcome-side proxies means the confounder was given several shadows and the number rests on all of them; a term naming a covariate alongside a proxy means the bridge was allowed to be a DIFFERENT function of that proxy in each stratum, which an additive covariate term does not do. Where `covariates` is non-empty, every identification claim holds within a level of C and the effect is averaged over C — say so, because it changes what the number is about. |
