...
2. **The caveats** — the gaps in `data_gap_report.gaps[]` whose `kind`
   ...
   | `proxy_coarsening_undeclared` | A proximal query's proxies show more levels than the states it posits for the unobserved confounder, and nobody has said which levels go together. Folding them is allowed; which fold is not something the data answers, and two folds give two numbers — so there is no estimate until someone chooses |
   | `regularisation_is_moving_the_answer` | A continuous-proxy proximal query solves an equation that has no numeric solution without a stabilising penalty, and re-solving at lighter penalties moves the answer further than sampling noise does — or gives no answer at all. The number is real and re-derivable; what it is partly a fact about is the penalty, so report which λ produced it and who chose it |
   | `treatment_bridge_leaves_its_range` | A proximal query's TREATMENT bridge is a reciprocal probability, so it is at least one everywhere — and the sieve solving for it is linear in its parameters and cannot know that. On this sample the fit came out negative on part of the rows, which means the inverse-probability average is weighting some rows negatively. Say the share, and say the two moves: a wider treatment-bridge span, or the doubly robust answer, which does not divide by that bridge alone and is already on the envelope |
   | `answer_is_a_test_not_an_effect_size` | A discrete proximal query whose channel could not be inverted, so what came back is a TEST of whether there is any effect (`numeric_estimate.no_effect_test`) rather than a number for it. Lead with the shape, not the p-value: a reader who asked how much and is handed whether will read a small p as a small effect. Rejecting is evidence of causation with no statement of size, sign or subgroup; failing to reject is not evidence of no effect, and the same result comes from a genuine null, from too few rows, and from proxies too weak to see one. Say which of the two reasons blocked the number, because the moves differ — a channel that will not invert is a measurement to go and make, and a treatment with more than two arms is answered by the `bridge` channel Themis already has |
...
| `structural_result.value` | A boolean whose *proposition* depends on `query_kind`. Where the question was about the graph — does X cause Y, are they associated, is the effect identifiable — the proposition is the answer, so state it. **State the proposition, not a paraphrase of it**: every one of these is faithful to the graph as supplied and to nothing else — a `cause` verdict is reachability over the edges the caller drew, so "yes, X causes Y" is a claim about the world that the kernel never made, while "a directed path runs from X to Y in this graph" is what it checked. The scoped sentence is on the envelope; do not unscope it. Where the question asked for a **quantity**, the same boolean asserts only that the estimand is identifiable: that is a precondition for the answer, never the answer, and rendering it as one tells a reader who asked "how large" that the result is "yes". When it is `false` there, "not identifiable" **is** the answer — say that, not "no" |
...
| `numeric_estimate.bootstrap` | What the interval above is a quantile OF: `requested` replicates asked for, `used` that a refit could use, and `discarded` naming what ate the rest by refusal species. Present on every bootstrapped estimate. `used` well short of `requested` says this sample or this declaration sits near a boundary the estimator cannot cross — a truncated interval and a full one are the same two numbers on the page, so read this before presenting the interval as settled. `kind: cluster` means whole clusters were drawn, and then the effective sample is the number of clusters, not of rows |
...
| `proximal_bridge` | the SAME identification argument as `proximal_matrix` for CONTINUOUS proxies, where there is no channel matrix to invert. What is solved for instead is one or two BRIDGE FUNCTIONS, each assumed to lie in the span of a design the caller declared, and `estimator` says which combination of them the point is: the outcome regression rests on the outcome bridge's span alone, the inverse-probability answer on the treatment bridge's alone, and the doubly robust one on neither individually — it is right if EITHER span holds and wrong, silently, only when both fail. Which of the three ran is therefore which assumption the reader is being asked to accept, and a report that names the method without naming that assumption has left out what the number means. Where a second bridge was declared, all three answers are on the envelope: they are the same quantity under different assumptions, so their agreement or disagreement is evidence about the assumptions and worth saying. A bridge equation is ill-posed — no numeric solution without a stabilising penalty — so a penalty λ is ALWAYS in the answer, one per bridge, either the caller's or one the estimator scaled to that equation. Say which, say what it was, and read `penalty_ladder` in the derivation before calling the number stable: it re-solves at four penalties spanning eight orders of magnitude, and the gap `regularisation_is_moving_the_answer` fires where the answer bends further than sampling noise. A span is an assumption about function shape rather than about the graph — more data does not fix a bridge outside it, so do not present the point as non-parametric. A design is a SUM OF TERMS and a term is a tensor product of one basis per variable it names, so read it before describing the answer: several proxies on a side means the confounder was given several shadows and the number rests on all of them; a term naming a covariate alongside a proxy means the bridge was allowed to be a DIFFERENT function of that proxy in each stratum, which an additive covariate term does not do. Where `covariates` is non-empty, every identification claim holds within a level of C and the effect is averaged over C — say so, because it changes what the number is about. |
| `proximal_null_test` | NOT an effect at all — the discrete channel could not be inverted, and what ran instead is Miao 2018 §4's test of the causal null `H0 : X ⊥ Y \| U`. There is no `point`, no interval and no do-risk on the envelope; the answer is `no_effect_test` (`statistic`, `degrees_of_freedom`, `p_value`, `coefficients`). The identification argument is the SAME model (f) as `proximal_matrix` — what was dropped is the invertibility of the channel, which is a fact about the sample, never the graph. Report it as a yes/no about existence and never as an effect size that came out small: rejecting says an effect exists and says nothing about how much, in which direction, or for whom; failing to reject is the absence of evidence rather than evidence of absence, and is equally produced by a genuine null, by too few rows, and by proxies too weak to see an effect. The gap `answer_is_a_test_not_an_effect_size` carries why no number was possible and what would buy one — quote it rather than inventing a reason, since the two reasons send the reader to different places. |
