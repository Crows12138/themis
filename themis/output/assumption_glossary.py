...
- **severity** — "if this is false, how does the conclusion die":
  ``invalidating`` (the number is not a causal effect at all) /
  ``distorting`` (shape or magnitude skewed, the average often survives) /
  ``confidence_only`` (only the interval moves);
- **testable** — can the reader do something to check it.

The default matters more than the table. An ID nobody has classified is
presumed to be an *identification* assumption at ``invalidating`` severity and
...
_EXACT: dict[str, _Entry] = {
    ...
    "ci_via_percentile_bootstrap": (
        _CI, _CONF, True, "置信区间由百分位 bootstrap 求得"),
}
...
_PREFIX: tuple[tuple[str, _Entry], ...] = (
    ("ci_via_pairs_cluster_bootstrap_on_",
     (_CI, _CONF, True, "置信区间由按 {} 重采样整簇的 pairs cluster bootstrap 求得")),
    ("ci_not_cluster_robust_econml_dml_interval_ignores_",
     (_CI, _CONF, True, "置信区间不是簇稳健的：解析区间忽略了 {} 的簇内相关，可能偏窄")),
    ("cluster_robust_influence_variance_on_",
     (_CI, _CONF, True, "影响函数方差按 {} 做了簇稳健修正")),
    ("propensity_clipped_to_floor_",
     (_FORM, _DIST, True, "倾向得分被截断到下限（{}）")),
    ("differential_misclassification_by_covariate_",
     (_ID, _INVAL, False, "差异误分类：误分类率随协变量 {} 而变，逐层用本层矩阵求逆")),
    ("zminus_reweighting_from_unbiased_reference_",
     (_ID, _INVAL, False, "Z⁻ 的重加权取自无偏参照样本（{}）")),
    ("zplus_weights_from_unbiased_reference_",
     (_ID, _INVAL, False, "Z⁺ 的权重取自无偏参照样本（{}）")),
    ...
    ("mtr_",
     (_ID, _INVAL, False, "单调处理响应（{}）：把无假设界的一侧收紧")),
    ...
    ("聚类 bootstrap", (_CI, _CONF, True, "")),
    ("经典加性测量误差", (_ID, _INVAL, False, "")),
    ("被经典加性误差污染的", (_ID, _INVAL, False, "")),
    ("后门可识别", (_ID, _INVAL, False, "")),
)
