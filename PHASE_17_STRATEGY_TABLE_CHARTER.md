...
**（slice 5 已拆除**：这不再是巧合而是一条声明——报告列出了它读的五个
extension 键，两个 recovery 块不在其中；哪天有 classifier 要读，加进 `reads`
即可，排序自动改。**）**
...
| 9 个 `_attach_*` 的手工顺序 + 注释里的理由 | 后处理器声明 `reads`/`writes`，顺序由**拓扑排序**得出；读一个尚未被写的块 = **启动期报错**，不是运行时沉默 ——**slice 5 已做，但这句需更正**：该错误状态被排序**消掉了**（无可报之错），启动期报错留给真正无法排序的表，见 slice 5 产出 |
| 块登记表 | **slice 6 已做，方向反过来了**：`themis/blocks.py` 声明块，策略/后处理引用它——因为「谁写这个块」的产出方跨四层（识别、估计、后处理、输出），从 `produces` 单向派生覆盖不到；登记表反过来当词汇表，两个单一出口拒绝未登记的块 |
| 可达性矩阵 | ~~由 `applies_when × layer` 派生~~ **没有对象，未建**：仓里从来没有一张被维护的可达性矩阵；结构那半 slice 3 的 `routing.bind` 已兑现，经验那半只能测（实测 27 格全非空）。见 slice 6 产出 |
| 假设账本 | **slice 6 已做，位置比原话低一层**：`assumes` 不能挂在策略行上——哪些假设成立取决于估计器内部走了哪个分支，行不知道。改为每条结构化 spec 声明它复述的 flat `id`，账本按 id 折叠。见 slice 6 产出 |
...
- ~~**slice 5 — 后处理拓扑化。**~~ **已完成**：10 个 pass 声明 reads/writes，
  顺序由拓扑排序得出，五类无法排序的表在 import 期拒绝（见 slice 5 产出）。
- ~~**slice 6 — 派生面。**~~ **已完成**：块登记成一等对象（`themis/blocks.py`）、
  假设账本的孪生关系改为估计器声明、元测试「跨入口同一估计量给同一个数」落地。
  **可达性矩阵没有建**——实测它不是一件被单独维护的东西，见 slice 6 产出。
...
**同一份记录顺带更正了一条先前登记**：slice 3 之后曾登记「发现 D 合并后
没有测试再产出 `identification:not_identifiable`（未断言其已死）」——
实测它**在覆盖到的 23 个里**，语料确实会走到。登记时的谨慎（没断言死）是对的，
那个怀疑本身是错的。
...
slice 4 先把现状钉住（`test_missing_item_species.py`），使那次更正是一次
**看得见的物种变更**，不是一次悄悄的编辑。

### 发现 G 的修复（2026-07-29 完成）—— 改物种把第二个错答案掀了出来

**现象**：把 `transport:{pop}` 的物种从 `missing_structural_input` 改成
`unidentifiable_no_admissible_set`，`answer_tier` 不再说 `point`——
**改说 `interval`**，而那个 interval 是 `manski_natural`。

**根因假设**：Manski 自然界限是从**观测联合分布**算出来的，那份联合分布属于
**源人群**；查询问的是 `real_world`。选择图刚刚判定源效应**不可迁移到目标
人群**——也就是说源分布对目标估计量**没有约束**。所以那个区间不是「关于对的
量、松一点」，而是「关于错的量、还挺紧」：源里 `P(y|x)=0.9` 给出的界可以把
目标真值 0.1 完全排除在外。

**为什么是根因不是表象**：与发现 G 的第一半、以及发现 E 同一族——
**查询自己带着的一个事实（`target_population`）没有任何守卫去看它**。
`_attach_bounds_result` 检查了目标事件离散、干预臂离散、状态是
`NEEDS_INVESTIGATION`……唯独没有检查这些界限说的是**哪个人群**。
第一半（物种）修好之前，这一半被 `point` 挡着看不见——**一个错答案盖住了
另一个错答案**。

**结构性修改**：两处，各说一件事。
1. `transport:{pop}` 声明 `UNIDENTIFIABLE_NO_ADMISSIBLE_SET`——**这个估计量
   在这张图这些数据下不可点识别**，这是 tier 要读的那件事。
2. `_attach_bounds_result` 在 `query.target_population is not None` 时返回——
   **无假设的地板是源人群的地板**，换一个人群就没有无假设的地板。

修后：`answer_tier == "none"`、`bounds_result is None`、summary 逐字带着
kernel 的理由。**「什么都没有」是这个查询的真答案**，两个字段现在都这么说。

**代价，明说**：`UNIDENTIFIABLE_NO_ADMISSIBLE_SET` 渲染器上挂的是**按物种写
死的三条通用出路**（测混杂 / 做 RCT / 找工具变量）。前两条对 transport 大致
说得通（要测的是让 S-admissibility 成立的协变量；RCT 要在目标人群做），
第三条不对。**出路取决于识别为什么失败，而物种一个人说不出这件事**——这是
同一族的下一个对象（产生端知道怎么补，报告在猜），本档不做，登记。
...
## Slice 5 产出 — 后处理的顺序是算出来的（2026-07-29 完成）

`dispatch()` 尾部把答案交给一串后处理器：置信度轨迹、点识别失败时的界限、
缺口报告、必须披露的告诫。**它们之间的顺序承重**——framing 跑在 investigation
前面，investigation 就会看到一个非空的 request 元组、断定「别人已经产出过
了」、直接退出，把自己那些 missing item 本该产生的每一条 action 全部丢掉。

而这个顺序此前**只由九个连写的调用表达，理由写在上面一条注释里**。注释不被
任何东西查阅，所以插错位置的一个 pass 不是错误，是**一个悄悄不同的答案**：
它读到的是 dispatcher 留下的值，通常是 `None`，而且不会说自己跑早了。

### 声明什么，以及「读」的定义

`themis/runtime/postprocess.py`。一个 pass 声明它**需要已完成的块**（`reads`）
和它**可能改动的块**（`writes`），顺序由拓扑排序得出。两个字段的**交集自己
携带语义**，不需要第三个动词：

- 只在 `writes` 里 = **产出者**；
- 同时在两边 = **改写者**（我需要它已经在，并把它改了还回去）。

关键在于「读」的定义必须精确，否则声明既写不对也检查不了：

> **一个 `reads` 声明，是在断言「跑在这个块的写者前面会让我给出不同的答案」。**

产出者对自己的块做「已经有了就不重复产出」的守卫**不是读**——那读的是
dispatcher 留下的状态，而每个 pass 看到的 dispatcher 状态都一样。这条区分不是
文字游戏：把它算作读，`investigation` 就会变成 `investigation_requests` 的第二
个改写者，于是这个块**没有产出者、有两个改写者**，表在 import 时就拒绝了。

### 块的粒度就是依赖存在的粒度

结果的每个字段是一个块，**`extensions` 除外**：它是一张 map，键与键之间毫无
关系。三个 pass 各写一个键，谁也不等谁。把整张 map 当成一个块，会在这三个之间
连出边，**读起来像一个有含义的顺序**。所以表里禁止命名整张 map，只能写
`extensions.<key>`——想读整张 map，就得把依赖的键列出来。

### import 期拒绝的五件事

| 拒绝 | 它挡住的是 |
|---|---|
| 块名不是 `QueryResult` 的字段 | 拼错的依赖，静默变成「没有依赖」 |
| 命名整张 `extensions` | 三个互不相干的 pass 被排出一个假顺序 |
| 一个块两个产出者 | 一个 pass 在覆盖另一个的成果 |
| 一个块两个改写者 | 谁先改由声明顺序决定——正是表要取代的东西 |
| 改写一个没人产出的块 | 需要块并把它改了还回去，却没有东西可改 |
| 环 | 没有任何顺序能同时满足 |

### 与本 charter 原话的一处出入（明说）

计划表里这一档写的是「**读一个尚未被写的块 = 启动期报错**」。实现下来这句
**不能照字面做**：`status` / `derivation` / `numeric_result` 这些块是
**dispatcher 产出的**，后处理器读它们完全正常，照字面禁止会把一半的合法读声
明判成错误。交付的是它可达的那一半，分成两条：

- **顺序保证**（不是报错）：凡是被某个 pass 写的块，排序保证它的写者排在每个
  读者之前——「读到尚未被写的块」这件事**不再可能发生**，所以没有可报的错。
- **启动期报错**：留给**真正无法排序**的表（上表五类）+ 块名根本不存在。

原话把两件事写成了一件。**结果比原话强的地方**：不是运行时沉默 → 启动期报错，
而是那个错误状态本身被排序消掉了。

### 算出来的顺序与手写的顺序只差一处，而那一处是修正

十个 pass（不是九个——`_attach_structural_caveats` 是**藏在
`_attach_data_gap_report` 尾调用里的第十个**）。按源码顺序声明，排序结果与手写
链条**逐位相同，只有一处例外**：

```
data_gap_report → reconcile_alt_paths → structural_caveats
```

手写链条里 `structural_caveats` 跑在 `reconcile_alt_paths` **之前**——因为它是
另一个 pass 的尾调用，**由构造决定，而不是由需要决定**。它读缺口报告，而
`reconcile_alt_paths` 改写缺口报告；读者读到的应当是**结果最终带出去的那一
版**。一个 pass 藏在另一个 pass 里，正是它当初怎么会读到过期版本的原因。

**slice 0 埋下的那个陷阱（缺口报告跑在两个 recovery 块之前，所以结构上看不见
它们）现在是一条声明而不是一个巧合**：报告声明了它读的五个 extension 键，
里面没有这两个。哪天某个 classifier 开始读它们，报告的 `reads` 里加一行，
排序会**自动**把两个 recovery pass 提到它前面。

### 声明是否完备，是跑出来的

顺序由声明算出，那声明漏一条读，表就是个带类型的注释。所以测试**不检查算出的
顺序等于某个列表**，而是检查**声明允许的每一种顺序都给同一个答案**：按依赖随
机采样合法线性序（定种子），每个语料程序跑 20 个。漏声明的读，会在某个把它排
到写者前面的序里露出来——**读表看不出来的，换个顺序跑就看出来了**。

三件配套的测量，都不是论证：

1. **20 个采样序两两不同，且没有一个等于规范序**——不是在反复跑同一条链。
2. **对照实验**：强行把 `structural_caveats` 排到报告之前（声明禁止的序），
   `explanation` 里的 ⚠ 行**消失**。没有这一条，上面那个测试在一张边毫无含义
   的表上同样会绿。
3. **覆盖率先测后补**：第一版语料下 `selection_recovery` 和
   `missing_data_recovery` **从未改动过任何结果**——它们是 no-op，重排 no-op
   不可能移动答案，等于两个 pass 的声明**没有任何东西在检查**。补了两个程序
   （对撞子上的样本限制 / 结局的缺失指示器）之后十个全部走活，并把这条测量
   **钉成守卫**：新加的 pass 若没有能走到它的语料程序，测试直接红。

### 顺带

`_attach_bounds_result` 有一个**从未被使用的 `bidirected` 参数**；统一签名后它
自然消失。`_attach_selection_recovery` / `_attach_missing_data_recovery` 里
`program is None or stmt is None` 的死守卫、以及 `_attach_data_gap_report` 里
「报告为 None 但结果上已有报告」这个**不可达分支**（该块只有一个产出者，现在
是表强制的）一并删除。

## Slice 6 产出 — 派生面（2026-08-03 完成）

三件里做了两件，第三件**实测发现没有对象**。

### 块登记：不是缺一张表，是那张表掉队了没人知道

信封有两种部件。带类型的是 `QueryResult` 的字段——声明一次，拼错是
`AttributeError`。其余的住在 `extensions` 里，一张无类型的 map，键是**在每个
写点和读点各拼一遍的字符串字面量**，拼错是**沉默**：读者拿到 `None`，然后报出
「这个块本来就没有」时会报的东西。

登记表**本来就存在**：`query_result.schema.json` 给其中 9 个块写了完整子
schema。但那张 map 明写「未知字段允许，为前向兼容」，于是**另外 11 个块这些年
陆续加进来、一个都没进登记表，没有任何东西说过一句**。这与本 charter 的总论
同型：一个没有一等表示的事实退化成约定，而漂移的约定不报错。

`themis/blocks.py` 把 20 个块各声明一次。`Block` 是 `str` 子类——块名用在原来
的一切地方都不变，但**只存在一处**。落进信封的仍是纯字符串：`__reduce__` 让
copy 与 pickle 退回 `str`，**这条是套件抓出来的**（deepcopy 一个带 `Block` 键
的信封当场炸，说明注册表元数据已经渗进数据）。

两个单一出口拒绝发出未登记的块。**读不对称**：`verify()` 仍接受别人信封里它不
认识的键——开放 map 就是为这个；**关的是我们发出去的东西**。

验证器保留字面量：与生产者共享符号等于自证。程序自己的 `extensions` 旁路也
保留——它与结果信封共用 `ambiguities` 这个词而已，这件事现在写下来了。

### 假设账本：孪生是被声明的，不是被推断的

原规则：**只要账本里有一条 identification 条目，估计器那张 flat 声明列表整张
都是它的复述，不读**。全量套件实测：**19 个族走这条路径**，而随它一起消失的是
它们在识别之外声明的一切——IPW 用哪种权重、TMLE 是定标代入估计、区间来自影响
函数还是聚类 bootstrap、近端识别依赖 Miao model f、因果概率答的是界因为没假设
单调性。**这些没有别处在说。**

判据不能靠细化来修：**唯一知道谁复述谁的是估计器自己**。所以每条结构化 spec
带上它所复述的那条 flat 声明的 `id`，折叠规则变成「**没有被任何条目按 id 认领
的声明，一律折入**」。一条把两件事写成一句的 spec 被拆开：一个 id 一条假设，
否则任何按 id 计数的东西都会看到一个洞。

**验证器独立写的那份，伸手够的是同一个逃生口。** 这是本仓「重复即正确性」第一
次实测失效——两份各自演化却收敛到同一个便利假设，所以 19 个族丢假设时两边都
不吭声。新规则不看 provenance，只问「声明过的每条 id 在不在账本上」，从信封本身
可判定。`outcome_error` 的豁免条款随之消失：它当初存在是因为孪生规则太粗。

**已知代价（未修）**：`mechanism_audit` 条目与被折入的 form 声明并排出现，文字
重叠。方向是多披露（该模块自己写明的安全方向）且替换的是**少**披露，但读起来
啰嗦。消除它要让 mechanism 条目声明它覆盖的 id 集合——那是另一个改动。

### 可达性矩阵：建之前先查，发现没有对象

charter 原话把它列在「现在 → 之后」表里，意思是它是一件被单独维护的东西。
**实测不是**：仓里没有任何可达性矩阵——不在代码里，不在文档里。而它想防的那
类缺陷已经有两道：

- **结构那半 slice 3 已兑现**：`routing.bind` 拒绝「表承诺了但本层没实现」与
  「实现了但表不往这里送」，**发现 D 正是这一类**。
- **经验那半只能测，不能从表推出来**：守卫是不透明谓词。带探针跑全量套件的
  实测结果是 **18 个估计行 × 9 个识别行、27 个格子全部非空**，没有死行；而
  任何一行真死掉，**它自己那个特性测试会先红**。

所以此刻建一张矩阵是**新增一件维护物**，与本 slice 的目的相反。原话按 slice 5
的先例更正在此，不假装兑现。

### 元测试：跨入口同一估计量给同一个数

`produces` 声明一行的数字是什么的估计。**在有东西去比这些数字之前，它宣称的一
致性从来不需要兑现**。`tests/test_estimand_agreement.py` 让四个入口在同一个查
询上落到表里两行，要求它们一致。

判据是量出来的，不是挑舒服的：四个点在给定数据下**是确定的**（重采样进的是区
间不是点），实测跨度 0.00102，界取五倍。**区间重叠是错的判据**——区间回答的是
「真值会不会在别处」，这里各约 0.042 宽，只要求各点落在别人区间里，等于接受两
行在一个 0.30 的效应上差 0.02。另有一条测试守着这个界本身：一旦区间窄到与界
同量级，这条测试就不再比区间多说什么，那时该重新量而不是把失败读成一致。

配套一条前提测试（四个入口确实落到四个不同方法，否则是在拿一个数跟自己比），
和一条说明**为什么允许比**的测试：`backdoor` 与 `doubly_robust` 声明同一个
estimand，IV 两行声明 `complier_effect`——**是表说了哪些数字可以比，不是读者的
判断**。
