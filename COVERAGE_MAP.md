...
> 更新时间：2026-05-07（iter 143 retract iter 141 Line 7 shortcut — math wrong）
...
| 2 | ADMG / 潜变量 | **~85%** | bidirected ✓ / m-sep ✓ / ADMG-backdoor/front-door ✓ / **Tian-Pearl ID Lines 1-6 ✓**（hedge witness + c-factor 乘积形式，2026-05-06）/ Line 7 完整 ID* 推迟（iter 141 尝试 shortcut → iter 143 retract：`_atom_to_target_va` 对 `state.x` 原子硬编码 do-value，使内层 Σ 退化；需要参数化 substitution machinery 才能正确处理）|
