...
| 8 | 测量误差 | **~70%** | **诊断 + 校正两层都在**。诊断：`measurement_error_concern` gap_kind 从程序结构（变量 `measurement` / `observability` 字段值含 self-report / 24h recall / single-occasion BP / questionnaire / FFQ / proxy 等已 documented 的高噪声模态）surfacing 风险，文献依据 MacMahon 1990 Lancet (regression dilution) / Hernán & Robins What If §9 / Fuller 1987。校正：**离散**=混淆矩阵求逆（结局 / 暴露 / 双信道 / 按臂或结局差异 / 按协变量差异 / **按二者联合差异**六种；#484 把「轴」从一列推广成列的元组，前两种差异型是它元组长度为一的特例，联合轴是三者中**最弱**的前提故账本记一条）；**连续线性结局**=regression calibration 矩量校正（暴露和/或后门协变量）；**连续非线性结局**=SIMEX 模拟外推（#463，Cook & Stefanski 1994，梯子是充分统计量故第二阶段逐数可重算，区间走 Stefanski-Cook 1995 τ 外推而非 bootstrap）；**连续结局**=不偏故只报精度代价；**Berkson 结构**=不校正、只报代价（#464，Berkson 1950，`X*=W+U` 下 `E[X*|W,Z]=W` 故朴素后门斜率本身就是因果斜率，校正它反而把真值 0.8 的答案改成 1.6；散布按 β̂²σ²_u 落进残差，是这一族里唯一被答案缩放的声明方差，故排在答案之后；结构无数据见证，靠 `structure: berkson` 声明，不认识的结构词也归这一行以免声明静默丢失）；**差异型**（#465，误差含一份随结局走的分量 `U=δ·Ỹ+f`，同时抬高观测方差**和**协方差，故矩量校正只除掉前者会落到真值另一侧——实测真值 0.8、给它正确的总方差，δ=−0.4 时它给 0.398、δ=+0.5 时给 0.900；闭式先把 δ·Var(Y|Z) 从协方差里减掉，`βx=(C−δB)/S`，δ=0 时精确退回 regression calibration，差 4.4e-15；δ 只能来自验证子研究，样本分不开 δ 与 βx）；**结局信道的差异型**（#485，`differential_outcome_correction`——记录到的结局含一份随暴露走的分量 `V=δ·X̃+f`，普通后门拟合把它整个吸进暴露的系数，估的是 βx+δ，故校正是**一次减法** `βx=naive−δ`，声明的 σ²_v 一点都不进点估计；非盲的结局评估者是最常见的来源。同时修好两处按**总**方差算的地方：装不装得进残差的那道闸和精度代价，δ 在场时都改按余量 σ²_f=σ²_v−δ²·Var(X|Z) 算——原先一份自洽的声明会被整条否掉）。未做：验证研究本身不确定性的传播、分类协变量。**「删失结局」原先挂在这一行的未做里，现在不在这里了**——它不是测量误差的一种：测量误差说记录到的值等于真值加噪声，删失说记录到的那一列压根不是那个量（是 `min(事件时间, 随访终点)`），见板块 11 |
...
| 11 | 连续 / 数据驱动估计 | **~55-65%** | **Phase 7.1-7.4 + Phase 14 已落地**（backdoor + front-door + IV + mediation numeric，4 条识别路径都能给数字 + CI；dose-response estimator 支持 LinearDML / CausalForestDML opt-in / DRLearner）/ **前门的中介现在可以是连续的**（#483；`frontdoor_empirical_{linear,logistic}`——P(M|X) 取自各臂自己的样本行而非拟合的多项 logistic 链，每行两次模型求值，不枚举分层因而也不受交叉积上限约束；线性形态的点由记录的结局系数 × 中介位移逐数重导。枚举路在可精确求和时仍然优先，已发布的数一个没动）/ **删失结局 = 限制平均生存时间**（#500，Kaplan-Meier 1958 + Irwin 1949 / Royston & Parmar 2013；`rmst_kaplan_meier`）——变量声明里新增 `censoring{event_indicator, horizon}` 这一条**正向声明**，声了才改路由、不声一如既往；逐（层×臂）非参数 KM 曲线 → 积到视界 τ → 按层权做 g-formula 标准化 → 两臂相减，方差走 Greenwood 经积分传播（Klein & Moeschberger §4.5）。**这条路是补一个被静默答错的量，不是加一个可选功能**：真值已知的设计上，记录列的臂间差和生存时间的臂间差差三分之一到三分之一（方向由删失率决定、页面上看不出来），而包返回 `numerically_solved`。τ 是问题的另一半不是设置——不给就诚实拒答（删失变量的无限制均值不是这份数据里的量）；τ 越过任一格的随访终点也拒答（越过去只能平推，是对没人看过那段的猜测）。**没做 Cox，且理由不是工作量**：风险比要比例风险假定、且回答的是另一个问题，而 RMST 不需要该假定、是时间量纲、无删失时精确退回普通臂间差。验证器从记录的风险集表独立重导曲线/面积/方差 |
| 12 | 因果发现 | **~56%** | **Phase 8.1 已落地**（PC/FCI/GES/GRaSP/LiNGAM via causal-learn + kernel_ast suggestion path）；**borrow-list #4：Markov blanket（grow-shrink 到不动点，连续/Fisher-Z + 离散/卡方 + **混合/条件高斯似然比**三条路径；#486，Lauritzen-Wermuth 1989 齐次 CG 族，充分统计量 = 每个被占据的离散格的 [计数, Σy, Σyy']，稀疏、上界是不同离散行数而非基数之积——**选它正是因为这条性质**：核方法的充分统计量是 n×n Gram 阵、秩方法的是秩，都等于原数据，而要携带原数据才能审的东西不是artifact；代价是一条前提「各格共用一个协方差」，而前提是这个包会记的东西。两个退化点就是另外两条路：无离散列时 G²=−n·log(1−ρ²)，无连续列时是同一个检验的偏差形式、自由度一致，故两条老路一位没动、由测试把这份一致钉死）+ 独立验证器 `verify_markov_blanket`**——发现层首个逐数验证（从记录的充分统计量[相关矩阵 / 稀疏联合列联表 / 逐格矩]重算完备性/最小性定义）；**时序 PCMCI（#449）+ NOTEARS（#462，自写：连续优化 + 可重算证书 + 逐边尺度稳健性）**——三个 artifact 各有独立验证器 |
...
| **DataGapReport schema (42 gap_kind / 3 severity / 6 ref_kind, Phase 10+13+iter expansions)** | **~100%** |
