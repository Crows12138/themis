...
当前全量测试基线：`1415 passed / 144 skipped`，warning-clean。
...
确认后再由 `apply_predicate_links(...)` / `apply_predicate_links_to_edges(...)`
回注变量和边端点；不自动改写模型语义。exercise case 也固定了低分
`waist_reduced -> belly_fat_loss` 候选：确认后 target framing 缩小，
但 `missing_distribution` 仍保留。A2 的
