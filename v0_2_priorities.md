...
> **当前版本指针**：本文件是 v0.1→v0.2 过渡期的 slice 排序记录。
> 系统已演化到 0.15.0-dev——v0.2 一阶段及之后的工作流强化（参数补
> 录闭环 / framing gate / data-gap report / KB+MCP bridge 等）已经
> 集成，活动优先排序由 [`ROADMAP.md`](ROADMAP.md) "现在最该做的事"
> 接管。本文件保留作为 v0.2 过渡期决策快照。
